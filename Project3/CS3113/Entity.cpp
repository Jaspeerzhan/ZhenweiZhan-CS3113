...
void Entity::update(float deltaTime, Entity *collidableEntities, int collisionCheckCount)
{
    ...
    // Integrate acceleration into velocity (Acceleration-based physics)
    mVelocity.x += mAcceleration.x * deltaTime;
    mVelocity.y += mAcceleration.y * deltaTime;

    // Add horizontal drag for drifting effect (makes drift more noticeable)
    const float HORIZONTAL_DRAG = 0.99f; // Higher = slower deceleration = more drift
    if (mAcceleration.x == 0.0f) {
        mVelocity.x *= HORIZONTAL_DRAG;
        // Only stop when velocity is very very small
