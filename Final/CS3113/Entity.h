...
enum EntityType   { PLAYER, BLOCK, PLATFORM, NPC, PROJECTILE, NONE };
enum AIType       { WANDERER, FOLLOWER, FLYER, CHARGER, WATCHER };
enum AIState      { WALKING, IDLE, FOLLOWING, PREPARING_CHARGE, CHARGING, CHARGING_COOLDOWN, AIMING, FIRING, COOLDOWN };
...
private:
    ...
    float mAttackDuration = 0.3f; // Attack lasts 0.3 seconds
    ...
    void AICharger(Entity *target);
    void AIWatcher(Entity *target);
    
    float mPatrolLeftBound = 0.0f;
    float mPatrolRightBound = 0.0f;
    bool mPatrolDirection = true; // true = right, false = left
    
    // Charger AI variables
    float mAIStateTimer = 0.0f;
    Vector2 mChargeDirection = {0.0f, 0.0f};
    
    // Watcher AI variables
    float mWatcherRange = 350.0f;
    bool mWatcherDamageApplied = false;
...
public:
    ...
    void attack()     { if (mAttackCooldown <= 0.0f) { mIsAttacking = true; mAttackCooldown = 0.5f; mAttackDuration = 0.3f; } }
    ...
    bool        shouldShoot()              const { return mShouldShoot;           }
    void        didShoot()                 { mShouldShoot = false;                }
    bool        canApplyWatcherDamage() const { return mAIType == WATCHER && mIsAttacking && !mWatcherDamageApplied; }
    void        markWatcherDamageApplied() { mWatcherDamageApplied = true; }
    ...
    void setAIType(AIType newType)
        { mAIType = newType;                       }
    void setDetectionRange(float range) { mWatcherRange = range; }
    void setPatrolBounds(float left, float right)
        { mPatrolLeftBound = left; mPatrolRightBound = right; }
};
