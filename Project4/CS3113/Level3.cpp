...
void Level3::initialise()
{
   ...
   mEnemyStartPosition = {mOrigin.x + 100.0f, mOrigin.y - 250.0f}; // Flyer starts in the middle area (moved down)
   ...
   mGameState.enemy = new Entity(
      {mOrigin.x + 100.0f, mOrigin.y - 250.0f},
   ...
   mGameState.enemy->setSpeed(Entity::DEFAULT_SPEED * 0.25f);
//    restruct the flyers movement
   mGameState.enemy->setPatrolBounds(mOrigin.x - 100.0f, mOrigin.x + 400.0f);
   mGameState.enemy->setColliderDimensions({
      enemyScale * 0.6f,
      enemyScale * 0.8f
   });
   mGameState.enemy->setAcceleration({0.0f, 0.0f}); // No gravity for flyer
...
void Level3::update(float deltaTime)
{
   ...
   float mapLeftBoundary = mOrigin.x - (LEVEL3_WIDTH * TILE_DIMENSION) / 2.0f;
   float mapTopBoundary = mOrigin.y - (LEVEL3_HEIGHT * TILE_DIMENSION) / 2.0f;
   float mapRightBoundary = mapLeftBoundary + (LEVEL3_WIDTH * TILE_DIMENSION);
   float mapBottomBoundary = mapTopBoundary + (LEVEL3_HEIGHT * TILE_DIMENSION);
   ...
   float playerHalfHeight = playerScale.y / 2.0f;
   ...
   // Restrict player from jumping above top edge of the map
   if (playerPos.y - playerHalfHeight < mapTopBoundary)
   {
       mGameState.player->setPosition({playerPos.x, mapTopBoundary + playerHalfHeight});
       Vector2 currentVelocity = mGameState.player->getVelocity();
       mGameState.player->setVelocity({currentVelocity.x, 0.0f}); // Stop upward velocity
   }
   
   playerPos = mGameState.player->getPosition();
   
   // Check if player fell off 
   constexpr float FAILURE_BUFFER = 128.0f;
   if (playerPos.y > mapBottomBoundary + FAILURE_BUFFER && 
   ...
   if (playerPos.x + playerHalfWidth > mapRightBoundary && mGameState.nextSceneID != -3)
   {
       mGameState.nextSceneID = -3; // Game win flag
       PlaySound(mGameState.levelCompleteSound); // Only play once
   }
...
void Level3::render()
{
   ...
   else if (mGameState.nextSceneID == -3)
   {
       ...
       int fontSize = 60;
       Vector2 textSize = MeasureTextEx(GetFontDefault(), winText, fontSize, 2.0f);
       Vector2 textPosition = {
           mOrigin.x - textSize.x / 2.0f,
           mOrigin.y - textSize.y / 2.0f
       };
       DrawTextEx(GetFontDefault(), winText, textPosition, fontSize, 2.0f, GREEN);
   }
}
